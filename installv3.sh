...
echo -e "${CYAN}${BOLD}"
echo "=================================================="
echo "🌸 SillyTavern-Termux 小红书专版 V3 🌸"
echo "💕 专为小红书姐妹们优化，零基础也能轻松安装"
echo "💖 快来和你的AI男朋友聊天吧~"
echo "✨ V3版本：双重下载策略，彻底解决menu.sh问题"
echo "🔧 加速源+GitHub官方，确保100%下载成功"
echo "=================================================="
echo -e "${NC}"
...
if [ ! -f "$MENU_PATH" ]; then
    ...
    # 如果GitHub官方也失败，报错退出
    if [ "$download_success" = false ]; then
        echo -e "${RED}${BOLD}>> ❌ 菜单脚本下载失败！${NC}"
        echo -e "${YELLOW}${BOLD}>> 💡 可能的解决方案：${NC}"
        echo -e "${CYAN}1. 检查网络连接是否正常${NC}"
        echo -e "${CYAN}2. 稍后重试安装${NC}"
        echo -e "${CYAN}3. 手动下载菜单脚本：${NC}"
        echo -e "${WHITE}   curl -k -fsSL -o ~/menu.sh https://github.com/nb95276/jiuguan/raw/main/menu.sh${NC}"
        echo -e "${WHITE}   chmod +x ~/menu.sh${NC}"
        echo -e "${CYAN}4. 或者使用VPN精简版：${NC}"
        echo -e "${WHITE}   curl -sL https://github.com/nb95276/jiuguan/raw/main/install_精简版v2.sh | bash${NC}"
        echo ""
        echo -e "${BRIGHT_MAGENTA}${BOLD}>> 🍻 如需帮助，请加群：877,957,256${NC}"
        exit 1
    fi
else
